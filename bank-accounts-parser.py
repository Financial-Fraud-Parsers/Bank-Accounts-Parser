...
class Bank:
    ...
    def check_bankname(self, content):
        ...
        #         print(i," : ", content_lower.index(i))
        #         # print the word the contains the bank name
        #         print("Word: ", content_lower[content_lower.index(i)-10:content_lower.index(i)+10])
        #         re.add(i)
        # return re
    
        for i in bank_list:
            if i.lower() in content_lower.lower():  # Check if the lowercase version is present in content_lower
                # print("content_lower: \n", content_lower)
                # print("i_lower: \n", i.lower())
                index = content_lower.index(i.lower())
                # Check if the matched word is a standalone word (not part of a longer word)
                if (index == 0 or not content_lower[index - 1].isalpha()) and \
                (index + len(i) == len(content_lower) or not content_lower[index + len(i)].isalpha()):
                    # print(i, ":", index)
                    # # print the word that contains the bank name
                    # word_start = max(0, index - 10)
                    # word_end = min(index + len(i) + 10, len(content_lower))
                    # print("Word:", content_lower[word_start:word_end])
                    re.add(i)
        print("Bank name re: \n", re)
        return re

    def check_type(self, content):
        acc_type = ["saving", "checking", "credit", "chkg"]
        re = set()
        content_lower = content.lower()
        for i in acc_type:
            if i in content_lower:
                ...
                # #print the index of the account type
                # print("Index of account type: ", content_lower.index(i))
                # print()
                # print(re)
        return re
    ...
    def check_amount(self, content):
        ...
        #make result a dictionary of list
        result = {"amount":[],"Credited_sum":[],"Debited_sum":[]}
        ...
        for i in re.finditer(r"\d{1,3}(?:,\d{3})*(?:\.\d+)+", content):
            ...
            if("+" in check_trans):
                ...
                # append the amount to the list
                # if the amount doesn't exist in the list then add to the list
                if i.group() not in result["amount"]:
                    result["amount"].append(i.group())
    ...
    def data_assign_byrow(self, photo_id, date, bank, account_type, amount, balance_cnt,writer):
        ...
        amount[0]["amount"].sort(reverse=True)
        # print ("amount: ", amount)
        # amount = amount[:balance_cnt]
        num_banks = len(bank)
        num_acc_type = len(account_type)
        # print("num_acc_type: ", num_acc_type)
        num_amount = len(amount[0]['amount'])
        # num_amount = 1
        # print("num_amount: ", num_amount)
        if num_banks == 1:
            # print ("num_banks == ", num_banks)
            # for i in range(0, max(num_banks,num_acc_type,num_amount)):    
            amt = amount.pop(0)
            # for i in range(num_acc_type):
            # for i in range(num_acc_type):
            for i in range(num_amount):
                ...
                if num_amount > 0:
                    ...
                    # amt = amount.pop(0)
                    print("amt: ", amt)
                    avail_amount=amt['amount'].pop(0)
                    # avail_amount=amt['amount']
                    print("avail_amount: ", avail_amount)
                    print('amt: ',amt)
...
if __name__ == '__main__':
    ...
    #     writer.writerow(row)
        ...
        for text_doc in textdoc_paths:
            ...
            # print("Content: \n",content)
            # print ("text_des: \n",text_des)
            # #print type os text_des
            # print("Type of text_des: ",type(text_des))

            if content:
                info_bank_type = check.check_type(text_des)
                print(info_bank_type)
